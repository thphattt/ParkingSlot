...
const ResidentFormModal = ({ isOpen, onClose, resident, onSuccess }) => {
  ...
  const [form, setForm] = useState({
    ...
    apartment: '', building: 'A', status: 'active', note: '',
  });
  ...
  return (
    ...
    >
      ...
      <form id="resident-form" onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2">
        {/* Họ tên — full width */}
        ...
        <div>
          ...
          <input name="phone" value={form.phone} onChange={handleChange} required placeholder="0901234567"
            className="w-full rounded-2xl border border-border bg-muted px-4 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring/20" />
        </div>
        ...
        {/* CCCD */}
        <div>
          <label className="mb-1.5 block text-sm font-medium text-muted-foreground">CCCD/CMND *</label>
          <input name="idCard" value={form.idCard} onChange={handleChange} required placeholder="012345678901"
            className="w-full rounded-2xl border border-border bg-muted px-4 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring/20" />
        </div>

        {/* Căn hộ */}
        <div>
          <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Căn hộ *</label>
          <input name="apartment" value={form.apartment} onChange={handleChange} required placeholder="1201"
            className="w-full rounded-2xl border border-border bg-muted px-4 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring/20" />
        </div>

        {/* Tòa nhà */}
        <div>
          <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Tòa nhà</label>
          <input name="building" value={form.building} onChange={handleChange} placeholder="A"
            className="w-full rounded-2xl border border-border bg-muted px-4 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring/20" />
        </div>
