...
const VehicleFormModal = ({ isOpen, onClose, vehicle, onSuccess }) => {
  ...
  const [form, setForm] = useState({
    licensePlate: '', type: 'car', brand: '', model: '',
    color: '', owner: '', status: 'active', note: '',
  });
  ...
  useEffect(() => {
    if (isOpen) {
      residentService.getAll({ limit: 100, status: 'active' })
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}
      title={isEdit ? 'Chỉnh sửa phương tiện' : 'Thêm phương tiện'} footer={footer} size="lg">
      ...
      <form id="vehicle-form" onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2">
        {/* Biển số */}
        <div>
        ...
        {/* Loại xe */}
        <div>
          <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Loại xe *</label>
        ...
        <div className="md:col-span-2">
          <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Chủ xe (cư dân) *</label>
        ...
        {/* Hãng xe */}
        <div>
          <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Hãng xe</label>
          <input name="brand" value={form.brand} onChange={handleChange} placeholder="Toyota, Honda..."
            className="w-full rounded-2xl border border-border bg-muted px-4 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring/20" />
        </div>

        {/* Dòng xe */}
        <div>
          <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Dòng xe</label>
          <input name="model" value={form.model} onChange={handleChange} placeholder="Camry, SH 150i..."
            className="w-full rounded-2xl border border-border bg-muted px-4 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring/20" />
        </div>

        {/* Màu xe */}
        <div>
          <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Màu xe</label>
          <input name="color" value={form.color} onChange={handleChange} placeholder="Trắng, Đen..."
            className="w-full rounded-2xl border border-border bg-muted px-4 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring/20" />
        </div>
        ...
        {/* Ghi chú */}
        <div className="md:col-span-2">
          <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Ghi chú</label>
