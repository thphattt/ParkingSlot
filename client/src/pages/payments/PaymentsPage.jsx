...
import { CreditCard, Plus, Search, Check, Trash2, Loader2, Calendar, DollarSign } from 'lucide-react';
...
const PaymentsPage = () => {
  ...
  const handleGenerate = async () => {
    setGenerating(true);
    try {
      const { data } = await paymentService.generate({ month: genMonth, year: genYear });
  ...
  const handlePay = async () => {
    ...
    setPaying(true);
    try {
      await paymentService.markAsPaid(payModal._id, { paymentMethod: payMethod });
  ...
  const columns = [
    {
      ...
      render: (val) => val ? (
        <div>
          ...
          <p className="text-xs text-muted-foreground">{val.building}-{val.apartment}</p>
        </div>
      ) : '—',
    },
    {
    ...
    },
    {
      ...
      label: 'Số tiền',
      render: (val) => (
        <span className="font-semibold text-foreground">{formatMoney(val)}</span>
      ),
    },
    {
      key: 'status',
    ...
    },
    {
      ...
      render: (val, row) => (
        <div className="flex items-center justify-end gap-1">
          {row.status === 'pending' && (
            <button onClick={() => { setPayModal(row); setPayMethod('cash'); }}
              title="Thanh toán"
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <div className="rounded-2xl border border-border bg-card p-4">
          ...
          <p className="text-xs text-muted-foreground">{formatMoney(totalPending)}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-4">
          <p className="text-sm text-muted-foreground">Đã thanh toán</p>
          <p className="text-2xl font-bold text-emerald-600">{countPaid}</p>
          <p className="text-xs text-muted-foreground">{formatMoney(totalPaid)}</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-4">
        ...
        <div className="rounded-2xl border border-border bg-card p-4">
          ...
          <p className="text-2xl font-bold text-red-500">{formatMoney(totalPending)}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select value={month} onChange={(e) => { setMonth(Number(e.target.value)); setPagination((prev) => ({ ...prev, page: 1 })); }}
        ...
        <select value={year} onChange={(e) => { setYear(Number(e.target.value)); setPagination((prev) => ({ ...prev, page: 1 })); }}
          className="rounded-2xl border border-border bg-muted px-4 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring/20">
      ...
      <DataTable columns={columns} data={payments} loading={loading}
        emptyMessage="Chưa có hóa đơn" emptyIcon={CreditCard} />
      ...
      {/* Generate Modal */}
      {generateModal && (
        <Modal isOpen={generateModal} onClose={() => setGenerateModal(false)}
          ...
          <div className="space-y-4">
            <div className="rounded-2xl bg-blue-500/10 px-4 py-3 text-sm text-blue-600">
              💡 Hệ thống sẽ tạo hóa đơn cho tất cả hợp đồng đang active. Hóa đơn đã tồn tại sẽ được bỏ qua.
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Tháng</label>
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Năm</label>
      ...
      {payModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative mx-4 w-full max-w-sm rounded-3xl border border-border bg-card p-6 shadow-2xl">
            <h3 className="text-lg font-semibold text-foreground mb-2">Xác nhận thanh toán</h3>
            <div className="space-y-3 mb-6">
              <p className="text-sm text-muted-foreground">
                Cư dân: <strong className="text-foreground">{payModal.resident?.fullName}</strong>
              </p>
              <p className="text-sm text-muted-foreground">
                Số tiền: <strong className="text-foreground">{formatMoney(payModal.amount)}</strong>
              </p>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-muted-foreground">Phương thức</label>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-sm rounded-3xl border border-border bg-card px-6 pb-6 pt-4 shadow-2xl">
            <h3 className="text-lg font-semibold text-foreground mb-2">Xóa hóa đơn</h3>
            <p className="text-sm text-muted-foreground mb-6">
              Xóa hóa đơn của <strong>{deleteConfirm.resident?.fullName}</strong> — {formatMoney(deleteConfirm.amount)}?
            </p>
            <div className="flex justify-end gap-3">
